<?php
$sqldata = "select groupaccessid,groupname,description  
		from groupaccess 
		where recordstatus = 1 and groupname like '%".(isset($_REQUEST['groupname']) ? $_REQUEST['groupname']
		: '')."%'";
...
$product = new CSqlDataProvider($sqldata,
	...
	'sort' => array(
		'attributes' => array(
			'groupaccessid', 'groupname', 'description'
		),
		'defaultOrder' => array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'groupname': $("input[name='<?php echo $this->IDField; ?>_search_groupname']").val()
			}});
		return false;
	}
  ...
  <div class="modal-dialog modal-lg">	
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-4">
            <label class="control-label" for="<?php echo $this->IDField; ?>_search_groupname"><?php echo getCatalog('groupname') ?></label>
          </div>
          <div class="col-md-8">
            <div class="input-group">
              <input type="text" name="<?php echo $this->IDField; ?>_search_groupname" class="form-control">
              <span class="input-group-btn">
