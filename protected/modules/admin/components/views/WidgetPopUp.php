<?php
$sqldata = "select a.widgetid,a.widgetname,a.widgettitle,a.description, b.modulename 
		from widget a 
		join modules b on b.moduleid = a.moduleid 
		where a.recordstatus = 1 
		and a.widgetname like '%".(isset($_REQUEST['widgetname']) ? $_REQUEST['widgetname'] : '')."%'
...
$product = new CSqlDataProvider($sqldata,
	...
	'sort' => array(
		'attributes' => array(
			'widgetid', 'widgetname', 'widgettitle', 'description', 'modulename'
		),
		'defaultOrder' => array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'widgetname': $("input[name='<?php echo $this->IDField; ?>_search_widgetname']").val(),
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_widgetname"><?php echo getCatalog('widgetname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField; ?>_search_widgetname" class="form-control">
							<span class="input-group-btn">
