<?php
$sqldata = "select distinct a.unitofmeasureid,a.uomcode,a.description
		from unitofmeasure a 
		inner join productplant b on b.unitofissue = a.unitofmeasureid
		where a.recordstatus = 1 
			and b.productid = ".(isset($_REQUEST['productid']) ? $_REQUEST['productid'] : 'null')."
			and a.uomcode like '%".(isset($_REQUEST['uomcode']) ? $_REQUEST['uomcode'] : '')."%'
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'uomcode': $("input[name='<?php echo $this->IDField ?>_search_uomcode']").val(),
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_search_uomcode"><?php echo getCatalog('uomcode') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_search_uomcode" class="form-control">
							<span class="input-group-btn">
