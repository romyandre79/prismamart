<?php
$sqldata	 = "select ownershipid,ownershipname
		from ownership 
		where recordstatus = 1 and ownershipname like '%".(isset($_REQUEST['ownershipname'])
		? $_REQUEST['ownershipname'] : '')."%'";
$count		 = count(Yii::app()->db->createCommand($sqldata)->queryAll());
$ownership = new CSqlDataProvider($sqldata,
	array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'ownershipname': $("input[name='<?php echo $this->IDField; ?>_search_ownershipname']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_ownershipname"><?php echo getCatalog('ownershipname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField; ?>_search_ownershipname" class="form-control">
							<span class="input-group-btn">
