...
$sqldata = "select a.addressbookid,a.fullname,a.creditlimit,a.currentlimit
		from addressbook a 
		where a.recordstatus = 1 and a.iscustomer = 1 and a.fullname like '%".(isset($_REQUEST['fullname'])
		? $_REQUEST['fullname'] : '')."%'";
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'fullname': $("input[name='<?php echo $this->IDField ?>_search_fullname']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_search_fullname"><?php echo getCatalog('fullname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_search_fullname" class="form-control">
							<span class="input-group-btn">
								<button name="<?php echo $this->IDField ?>SearchButton" type="button" class="btn btn-primary" onclick="<?php echo $this->IDField ?>searchdata()"><span class="fa fa-ban"></span></button>
							</span>
						</div>
					</div>
				</div>
