<?php
$sqldata = "select a.taxid,a.taxcode,a.taxvalue
		from tax a 
		where a.recordstatus = 1 and a.taxcode like '%".(isset($_REQUEST['taxcode']) ? $_REQUEST['taxcode']
		: '')."%'
		and a.taxvalue like '%".(isset($_REQUEST['taxvalue']) ? $_REQUEST['taxvalue']
		: '')."%'";
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'taxcode': $("input[name='<?php echo $this->IDField ?>_search_taxcode']").val(),
				'taxvalue': $("input[name='<?php echo $this->IDField ?>_search_taxvalue']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_search_taxcode"><?php echo getCatalog('taxcode') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_search_taxcode" class="form-control">
							<span class="input-group-btn">
				...
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_search_taxvalue"><?php echo getCatalog('taxvalue') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_search_taxvalue" class="form-control">
						</div>
					</div>
				</div>
				<?php
				$this->widget('zii.widgets.grid.CGridView',
					...
					'columns' => array(
						array(
							'header' => getCatalog('taxid'),
							'name' => 'taxid',
							'value' => '$data["taxid"]',
							'htmlOptions' => array('width' => '1%')
						),
						array(
							'header' => getCatalog('taxcode'),
							'name' => 'taxcode',
							'value' => '$data["taxcode"]',
						),
						array(
							'header' => getCatalog('taxvalue'),
							'name' => 'taxvalue',
							'value' => '$data["taxvalue"]',
						),
					),
