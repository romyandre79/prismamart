...
$sqldata = "select a.addresstypeid,a.addresstypename
		from addresstype a 
		where a.recordstatus = 1 and a.addresstypename like '%".(isset($_REQUEST['addresstypename'])
		? $_REQUEST['addresstypename'] : '')."%'";
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'addresstypename': $("input[name='<?php echo $this->IDField ?>_search_addresstypename']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_addresstypename"><?php echo getCatalog('addresstypename') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_addresstypename" class="form-control">
							<span class="input-group-btn">
