	...
	inner join address b on b.addressbookid = a.addressbookid')->queryAll();
		foreach ($companys as $company) {
			...
			$marker = new EGMapMarkerWithLabel($company['lat'], $company['lng'],
				...
				'icon' => $icon));
			if ($company['isvendor'] == 1) {
				$info_window = new EGMapInfoWindow(getCatalog('fullname').': '.$company['fullname'].
					'<br/>'.getCatalog('address').': '.$company['addressname'].
					'<br/>'.getCatalog('currentdebt').': '.Yii::app()->format->formatCurrency($company['currentdebt']));
				$marker->addHtmlInfoWindow($info_window);
			} else {
				$info_window = new EGMapInfoWindow(getCatalog('fullname').': '.$company['fullname'].
					'<br/>'.getCatalog('address').': '.$company['addressname'].
					'<br/>'.getCatalog('creditlimit').': '.Yii::app()->format->formatCurrency($company['creditlimit']).
					'<br/>'.getCatalog('currentlimit').': '.Yii::app()->format->formatCurrency($company['currentlimit']));
				$marker->addHtmlInfoWindow($info_window);
			}
