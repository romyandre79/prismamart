<?php
$sqldata = "select distinct a.productid,a.productpic,a.productname
		from product a 
		inner join productplant b on b.productid = a.productid 
		inner join sloc c on c.slocid = b.slocid
		where a.recordstatus = 1 and 
		a.productname like '%".(isset($_REQUEST['productname']) ? $_REQUEST['productname']
		: '')."%' and
		c.slocid like '%".(isset($_REQUEST['slocid']) ? $_REQUEST['slocid'] : '')."%'";
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				...
				'slocid': $("input[name='<?php echo $this->RelationID ?>']").val()
			}});
		return false;
	}
