<?php
$sqldata			 = "select materialtypeid,materialtypecode,description
		from materialtype 
		where recordstatus = 1 
		and materialtypecode like '%".(isset($_REQUEST['materialtypecode']) ? $_REQUEST['materialtypecode']
		: '')."%'
		and description like '%".(isset($_REQUEST['description']) ? $_REQUEST['description']
		: '')."%'";
$count				 = count(Yii::app()->db->createCommand($sqldata)->queryAll());
$materialtype	 = new CSqlDataProvider($sqldata,
	array(
	...
	'sort' => array(
		'attributes' => array(
			'materialtypeid', 'materialtypecode', 'description'
		),
		'defaultOrder' => array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'materialtypecode': $("input[name='<?php echo $this->IDField; ?>_search_materialtypecode']").val(),
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_materialtypecode"><?php echo getCatalog('materialtypecode') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField; ?>_search_materialtypecode" class="form-control">
							<span class="input-group-btn">
				...
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_description"><?php echo getCatalog('materialtypecode') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
