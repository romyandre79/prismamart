<?php
$sqldata				 = "select materialstatusid,materialstatusname
		from materialstatus 
		where recordstatus = 1 and materialstatusname like '%".(isset($_REQUEST['materialstatusname'])
		? $_REQUEST['materialstatusname'] : '')."%'";
$count					 = count(Yii::app()->db->createCommand($sqldata)->queryAll());
$materialstatus	 = new CSqlDataProvider($sqldata,
	array(
	...
	'sort' => array(
		'attributes' => array(
			'materialstatusid', 'materialstatusname'
		),
		'defaultOrder' => array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'materialstatusname': $("input[name='<?php echo $this->IDField; ?>_search_materialstatusname']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_materialstatusname"><?php echo getCatalog('materialstatusname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField; ?>_search_materialstatusname" class="form-control">
							<span class="input-group-btn">
