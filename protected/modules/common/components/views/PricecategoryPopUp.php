...
$sqldata = "select a.pricecategoryid,a.categoryname
		...
		where a.recordstatus = 1 and a.categoryname like '%".(isset($_REQUEST['categoryname'])
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'categoryname': $("input[name='<?php echo $this->IDField ?>_search_categoryname']").val()
			}});
		return false;
	}
...
<?php echo $this->IDField ?>ShowButtonClick();
		...
		$("input[name='<?php echo $this->IDField; ?>_categoryname']").keyup(function (e) {
			if (e.keyCode == 13) {
<?php echo $this->IDField ?>searchdata()
			}
		});
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField ?>_categoryname"><?php echo getCatalog('categoryname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField ?>_categoryname" class="form-control">
							<span class="input-group-btn">
