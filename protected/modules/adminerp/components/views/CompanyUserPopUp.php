<?php
$sqldata = "select a.companyid,a.companyname  
		from company a 
		where a.recordstatus = 1 and a.companyname like '%".(isset($_REQUEST['companyname'])
		? $_REQUEST['companyname'] : '')."%' 
		and companyid in (select gm.menuvalueid from groupaccess c
...
where upper(e.username)=upper('".Yii::app()->user->name."') and upper(ma.menuobject) = upper('company'))";
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'companyname': $("input[name='<?php echo $this->IDField; ?>_search_companyname']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->IDField; ?>_search_companyname"><?php echo getCatalog('companyname') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->IDField; ?>_search_companyname" class="form-control">
							<span class="input-group-btn">
