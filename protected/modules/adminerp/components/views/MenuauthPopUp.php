<?php
$sqldata	 = "select a.menuauthid,a.menuobject
		from menuauth a 
		where a.recordstatus = 1 
		and a.menuobject like '%".(isset($_REQUEST['menuobject']) ? $_REQUEST['menuobject']
		: '')."%'";
...
$product	 = new CSqlDataProvider($sqldata,
	...
	'sort' => array(
		'attributes' => array(
			'menuauthid', 'formatdoc', 'menuobject'
		),
		'defaultOrder' => array(
...
<script>
	function <?php echo $this->IDField ?>searchdata() {
		$.fn.yiiGridView.update("<?php echo $this->PopGrid ?>", {data: {
				'menuobject': $("input[name='<?php echo $this->IDField ?>_menuobject']").val()
			}});
		return false;
	}
...
<div id="<?php echo $this->IDDialog ?>" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">	
      ...
      <div class="modal-body">
				<div class="row">
					<div class="col-md-4">
						<label class="control-label" for="<?php echo $this->ColField ?>_search_menuobject"><?php echo getCatalog('menuobject') ?></label>
					</div>
					<div class="col-md-8">
						<div class="input-group">
							<input type="text" name="<?php echo $this->ColField ?>_search_menuobject" class="form-control">
							<span class="input-group-btn">
